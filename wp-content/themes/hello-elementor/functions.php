...
function portfolio_content_ajax_handler() {
    ...
    // 1. Deteksi tipe berdasarkan URL halaman asal (HTTP_REFERER)
    $referer_url = isset($_SERVER['HTTP_REFERER']) ? strtolower($_SERVER['HTTP_REFERER']) : '';
    $type_by_url = (strpos($referer_url, 'portofolio-video') !== false) ? 'video' : 'image';

    // 2. Prioritaskan klik tombol toggle ($_POST['type']) jika ada, kalau tidak pakai deteksi URL
    $type = (isset($_POST['type']) && !empty($_POST['type'])) ? $_POST['type'] : $type_by_url;

    // Validasi akhir agar nilainya hanya boleh 'video' atau 'image'
    $type = ($type === 'video') ? 'video' : 'image';

    // 3. Tentukan tabel database berdasarkan tipe media
    $table    = ($type === 'video') ? 'wp9y_portofolio_video' : 'wp9y_portofolio';
    $offset   = isset($_POST['offset']) ? intval($_POST['offset']) : 0;
    $search   = isset($_POST['search']) ? sanitize_text_field($_POST['search']) : '';
    ...
    // Reset seed acakan jika offset kembali ke 0 (ganti tab/kategori)
    $seed = isset($_POST['seed']) ? intval($_POST['seed']) : 0;
    if (!$seed || $offset === 0) {
        $seed = rand(1, 999999);
    }
    ...
    // Sorting berdasarkan Rating dan Seed Acak
    $query .= $wpdb->prepare(" ORDER BY CASE WHEN t.rating IS NULL THEN 0 ELSE t.rating END DESC, RAND(%d)", $seed);
    $query .= " LIMIT $offset, 12";
...
//     wp_register_style('portfolio-css', get_template_directory_uri() . '/css/portfolio.css', [], filemtime(get_template_directory() . '/css/portfolio.css'));
// });
...
.porto-card {
  ...
  /* Flexbox susunan vertikal (Image Wrap -> Content) */
  display: flex; 
  flex-direction: column;
  height: 100%; /* Memaksa card mengisi penuh tinggi baris grid container */
...
/* Card Content (Pembungkus Title & Footer) */
.porto-card-content {
  padding: 20px;
  display: flex; 
  flex-direction: column; 
  flex: 1; /* Mengambil semua sisa space kosong di dalam card */
  justify-content: space-between; /* Dorong title ke atas, footer ke paling bawah */
}
...
.porto-card-title {
  ...
  /* Batasi maksimal 2 baris teks, sisanya jadi (...) */
  display: -webkit-box; 
  -webkit-line-clamp: 2; 
  -webkit-box-orient: vertical; 
  overflow: hidden;
  
  /* flex: 1; <-- Di-remove karena space kosongnya sudah dihandle .porto-card-content */
}
...
@media (max-width: 600px) {
  ...
  .porto-section { padding-left: 24px; padding-right: 24px; }
    ...
    <main class="porto-section">
      ...
      <div class="porto-grid" id="portfolio-grid-container"></div>
    ...
    <script>
    var portfolio_ajax_obj = { ajax_url: "<?php echo admin_url('admin-ajax.php'); ?>" };

    jQuery(document).ready(function($) {
        // Deteksi tipe awal berdasarkan URL halaman saat ini
        let currentType = window.location.pathname.toLowerCase().includes('portofolio-video') ? 'video' : 'image';
        ...
        // Sinkronkan tampilan toggle button sesuai tipe yang terdeteksi dari URL
        $('.porto-toggle-btn').removeClass('active');
        $('.porto-toggle-btn').each(function() {
            let isVideoBtn = $(this).text().toLowerCase().includes('video');
            if ((currentType === 'video' && isVideoBtn) || (currentType === 'image' && !isVideoBtn)) {
        ...
        function fetchPortfolios(isAppend = false) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    $grid.css('opacity', '1');
                    if (isAppend) {
                        if (response.trim() === '') { $loadMoreBtn.hide(); }
                        else { $grid.append(response); currentOffset += 12; }
                    } else {
                        $grid.html(response);
                        currentOffset = 12;
                        $loadMoreBtn.show();
                    ...
                    if (newItemsCount < 12) { $loadMoreBtn.hide(); } else { $loadMoreBtn.show(); }
                },
                error: function() { $grid.css('opacity', '1'); }
            });
        }
...
function render_company_profile_shortcode() {
    ...
    $videos = $wpdb->get_results(
        "SELECT DISTINCT v.*, p.nama_panggilan, p.kode_nama
         ...
         WHERE v.status = 'approved' AND p.status = 'approved' AND sm.service_slug = 'jasa-pembuatan-video-company-profile'
         ORDER BY v.id DESC
         LIMIT 9"
    );
...
function render_event_production_shortcode() {
    ...
    $videos = $wpdb->get_results(
        "SELECT DISTINCT v.*, p.nama_panggilan, p.kode_nama
         ...
         JOIN wp9y_personel p ON v.personel_id = p.id
         JOIN wp9y_service_map sm ON sm.item_type = 'video' AND sm.item_id = v.id
         WHERE v.status = 'approved' AND p.status = 'approved' AND sm.service_slug = 'event-production-event-organizer'
         ORDER BY v.id DESC
         LIMIT 9"
    );
    ...
    $fotos = $wpdb->get_results(
        "SELECT DISTINCT p.*, ps.nama_panggilan, ps.kode_nama
         ...
         JOIN wp9y_personel ps ON p.personel_id = ps.id
         JOIN wp9y_service_map sm ON sm.item_type = 'foto' AND sm.item_id = p.id
         WHERE p.status = 'approved' AND ps.status = 'approved' AND sm.service_slug = 'event-production-event-organizer'
         ORDER BY p.id DESC"
    );

    // No fallback — if empty, show nothing
...
// ============================================================
// SERVICE MAP — Hubungkan Portofolio ke Halaman Layanan
// ============================================================
function wp9y_create_service_map_table() {
    global $wpdb;
    $table_name = 'wp9y_service_map';
...
add_action('admin_init', 'wp9y_create_service_map_table');

add_action('admin_menu', function() {
    add_submenu_page(
        'options-general.php',
        'Link Portofolio ke Halaman Layanan',
        'Service Link',
        'manage_options',
        'service-link',
        'wp9y_render_service_link_page'
    );
});

function wp9y_render_service_link_page() {
    global $wpdb;
    if (!current_user_can('manage_options')) wp_die('Unauthorized');

    if (isset($_POST['save_service_links']) && isset($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'], 'service_links_save')) {
        $wpdb->query("TRUNCATE TABLE wp9y_service_map");
        $links = $_POST['link'] ?? []; $order = 0;
        foreach ($links as $item_type => $items) {
            foreach ($items as $item_id => $slugs) {
                if (!is_array($slugs)) continue;
                foreach ($slugs as $slug) {
                    $wpdb->insert('wp9y_service_map', [
        ...
        echo '<div class="notice notice-success"><p>Service links updated!</p></div>';
    }

    $service_pages = [
        'jasa-pembuatan-video-company-profile' => 'Company Profile',
        'wedding-prawedding' => 'Wedding / Prewedding',
        'event-production-event-organizer' => 'Event Production',
        'dokumentasi-event' => 'Dokumentasi Event',
        'video-produk-branding-iklan' => 'Video Produk Iklan',
    ...
    $videos = $wpdb->get_results("SELECT v.*, ps.nama_panggilan, ps.kode_nama FROM wp9y_portofolio_video v LEFT JOIN wp9y_personel ps ON v.personel_id = ps.id ORDER BY v.id DESC");
    ?>
    <div class="wrap">
        <h1 class="wp-heading-inline">Link Portofolio ke Halaman Layanan</h1>
        <hr class="wp-header-end">
        <div style="background:#fff; border:1px solid #c3c4c7; padding:20px; margin-top:20px; border-radius:4px;">
        <form method="post">
            <?php wp_nonce_field('service_links_save'); ?>
            <table class="wp-list-table widefat fixed striped" id="service-link-table">
                <thead><tr>
                    <th width="40">No</th>
                    <th width="60">Tipe</th>
                    <th width="60">ID</th>
                    ...
                    <th>Creator</th>
                    <th>Thumbnail</th>
                    <?php foreach ($service_pages as $slug => $label): ?>
                    <th style="text-align:center;font-size:11px;"><?php echo esc_html($label); ?></th>
                    <?php endforeach; ?>
                </tr></thead>
                <tbody>
                <?php $no = 0;
                foreach ($fotos as $p): $no++;
                    $checked = $existing['foto'][$p->id] ?? [];
                    $thumb = !empty($p->foto_thumbnail) ? $p->foto_thumbnail : $p->foto_url;
                    $yt_id = '';
                    if (!empty($p->foto_url) && preg_match('/(?:youtube\.com\/(?:watch\?v=|shorts\/)|youtu\.be\/)([a-zA-Z0-9_-]+)/', $p->foto_url, $m)) $yt_id = $m[1];
                ?>
                    <tr>
                        <td><?php echo $no; ?></td>
                        <td><span style="background:#e5f5e5;padding:2px 6px;border-radius:3px;font-size:11px;">foto</span></td>
                        <td><?php echo $p->id; ?></td>
                        <td><?php echo esc_html($p->judul); ?></td>
                        <td><?php echo esc_html(($p->nama_panggilan ?? '-') . '-' . ($p->kode_nama ?? '')); ?></td>
                        <td><?php if (!empty($thumb)): ?><a href="#" onclick="event.preventDefault();openLxVideoModal('<?php echo esc_url($thumb); ?>');"><img src="<?php echo esc_url($thumb); ?>" style="width:80px;height:45px;object-fit:cover;border-radius:3px;cursor:pointer;"></a><?php else: ?><span style="color:#999;font-size:11px;">—</span><?php endif; ?></td>
                        <?php foreach ($service_pages as $slug => $label): ?>
                        <td style="text-align:center;"><input type="checkbox" name="link[foto][<?php echo $p->id; ?>][]" value="<?php echo $slug; ?>" <?php echo in_array($slug, $checked) ? 'checked' : ''; ?>></td>
                        <?php endforeach; ?>
                    </tr>
                <?php endforeach;
                foreach ($videos as $v): $no++;
                    $checked = $existing['video'][$v->id] ?? [];
                    $embed_url = '';
                    if (!empty($v->video_url) && preg_match('/(?:youtube\.com\/(?:watch\?v=|shorts\/)|youtu\.be\/)([a-zA-Z0-9_-]+)/', $v->video_url, $m)) {
                        $embed_url = "https://img.youtube.com/vi/{$m[1]}/mqdefault.jpg";
                    }
                ?>
                    <tr>
                        <td><?php echo $no; ?></td>
                        <td><span style="background:#e5f0ff;padding:2px 6px;border-radius:3px;font-size:11px;">video</span></td>
                        <td><?php echo $v->id; ?></td>
                        <td><?php echo esc_html($v->judul); ?></td>
                        <td><?php echo esc_html(($v->nama_panggilan ?? '-') . '-' . ($v->kode_nama ?? '')); ?></td>
                        <td><?php if (!empty($embed_url)): ?><a href="#" onclick="event.preventDefault();openLxVideoModal('<?php echo esc_url($v->video_url); ?>');"><img src="<?php echo esc_url($embed_url); ?>" style="width:80px;height:45px;object-fit:cover;border-radius:3px;cursor:pointer;"></a><?php else: ?><span style="color:#999;font-size:11px;">—</span><?php endif; ?></td>
                        <?php foreach ($service_pages as $slug => $label): ?>
                        <td style="text-align:center;"><input type="checkbox" name="link[video][<?php echo $v->id; ?>][]" value="<?php echo $slug; ?>" <?php echo in_array($slug, $checked) ? 'checked' : ''; ?>></td>
                        <?php endforeach; ?>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
            <p style="margin-top:15px;"><button type="submit" name="save_service_links" class="button button-primary">Simpan Semua Perubahan</button></p>
        </form></div></div>

    <style>
    .wrap table.wp-list-table.widefat thead th { position:sticky; top:32px; z-index:10; background:#fff; }
    .wrap table.wp-list-table td { vertical-align:middle; }
    </style>

    <div id="lxVideoModal" style="display:none;position:fixed;z-index:99999;left:0;top:0;width:100%;height:100%;background:rgba(0,0,0,0.9);" onclick="closeLxVideoModal()">
      <span style="position:absolute;right:20px;top:20px;color:#fff;font-size:30px;cursor:pointer;z-index:10;" onclick="closeLxVideoModal()">&times;</span>
      <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:90%;max-width:960px;">
        <iframe id="lxVideoFrame" src="" style="width:100%;aspect-ratio:16/9;border:none;border-radius:8px;" allowfullscreen></iframe>
      </div>
    </div>
    <script>
    var m = document.getElementById('lxVideoModal');
    var f = document.getElementById('lxVideoFrame');
    function openLxVideoModal(u) {
        if (!u) return;
        var id = u.match(/(?:youtube\.com\/(?:watch\?v=|shorts\/)|youtu\.be\/)([a-zA-Z0-9_-]+)/);
        f.src = id ? 'https://www.youtube.com/embed/' + id[1] + '?autoplay=1' : u;
        m.style.display = 'block';
    }
    function closeLxVideoModal() { f.src = ''; m.style.display = 'none'; }
    document.addEventListener('keydown',function(e){if(e.key==='Escape')closeLxVideoModal();});
    </script>
    <?php
}
